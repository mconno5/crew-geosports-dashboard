...
def render_dashboard(template_path: Path, output_path: Path, dashboard_data: dict) -> None:
    ...
    html = re.sub(
        r"const dailyScores = \{.*?\};\n\nconst dailyGroupAverage = \[.*?\];\n\nconst questionStats = \{.*?\};\n\nconst groupQuestionStats = \[.*?\];\n\nconst maxAvg",
        js_const("dailyScores", dashboard_data["dailyScores"])
        + "\n\n"
        + js_const("dailyGroupAverage", dashboard_data.get("dailyGroupAverage", []))
        + "\n\n"
        + js_const("questionStats", dashboard_data.get("questionStats", {}))
        + "\n\n"
        + js_const("groupQuestionStats", dashboard_data.get("groupQuestionStats", []))
        + "\n\nconst maxAvg",
        html,
        count=1,
        flags=re.DOTALL,
